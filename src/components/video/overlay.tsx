...
import { TextTrackSelector } from './text-tracks-selector'
import { ToggleControl } from './toggle-control'
import { TopControl } from './top-control'
import { VideoTrackSelector } from './video-tracks-selector'
    ...
    <>
      <View style={styles.topControls}>
        <View style={styles.resizeModeControl}>
          <TopControl srcListId={srcListId} showRNVControls={controls} toggleControls={toggleControls} />
        </View>
      </View>
      {!controls ? (
        <>
          ...
          <View style={styles.bottomControls}>
            <View style={styles.generalControls}>
              ...
              <ToggleControl isSelected={paused} onPress={togglePause} selectedText='pause' unselectedText='playing' />
              <ToggleControl
                isSelected={repeat}
            ...
            <View style={styles.generalControls}>
              ...
              <MultiValueControl values={[0, 0.25, 0.5, 1.0, 1.5, 2.0]} onPress={onRateSelected} selected={rate} />
              {/* shall be replaced by slider */}
              <MultiValueControl values={[0.5, 1, 1.5]} onPress={onVolumeSelected} selected={volume} />
              <MultiValueControl
                values={[ResizeMode.COVER, ResizeMode.CONTAIN, ResizeMode.STRETCH]}
                onPress={onResizeModeSelected}
                selected={resizeMode}
              />
              <ToggleControl isSelected={muted} onPress={toggleMuted} text='muted' />
              {isIos ? (
                <ToggleControl
                  isSelected={paused}
                  onPress={() => {
