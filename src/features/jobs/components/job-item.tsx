      ...
      <VStack className="gap-3 p-4">
        ...
        <HStack className="items-center gap-3">
          ...
          <VStack className="flex-1">
            {!!job.companyName && (
              <Text numberOfLines={1} className="text-base font-medium text-jego-foreground">
                {job.companyName}
              </Text>
            )}
            {(job.companyCity || job.companyState) && (
              <HStack className="items-center gap-1">
                <Icon as={IconMapPinFilled} size="sm" className="text-jego-muted-foreground" />
                <Text numberOfLines={1} className="text-xs text-jego-muted-foreground">
        ...
        <VStack className="gap-1">
          <Text numberOfLines={1} className="text-lg font-semibold text-jego-foreground">
            {job.title}
          </Text>
          {!!job.description && (
            <Text numberOfLines={2} className="text-sm text-jego-muted-foreground">
              {job.description}
            </Text>
          )}
        </VStack>
        ...
        {job.expiresAt && !isExpired && (
          <HStack className="items-center gap-1">
            ...
            <Text className="text-sm text-jego-muted-foreground">
              Expire le {formatDate(job.expiresAt)}
            </Text>
          </HStack>
        )}
        ...
        <HStack className="items-center justify-between">
          <Text className="text-xs text-jego-muted-foreground">Publié le {formatDate(job.createdAt)}</Text>
        </HStack>
      </VStack>
    </Card>
  )
