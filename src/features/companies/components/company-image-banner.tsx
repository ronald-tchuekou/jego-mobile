...
const CompanyImageBannerComponent = ({ company, className }: Props) => {
  ...
  return (
    <View
      className={cnBase(
        'w-full overflow-hidden rounded-t-xl bg-jego-accent border-b border-jego-border aspect-[4/1] relative',
        className,
      )}
    >
      {hasError ? (
        <View className='absolute inset-0 bg-muted flex items-center justify-center'>
          <Text className='text-jego-muted-foreground text-sm text-center p-4'>
