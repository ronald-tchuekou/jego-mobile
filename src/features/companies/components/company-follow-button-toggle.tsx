...
export const CompanyFollowButtonToggle = ({ companyId }: Props) => {
  ...
  return (
    <Button
      variant={'outline'}
      onPress={handleClick}
      isDisabled={isPending || loadingFollowing}
      className={'flex-none rounded-xl bg-background border-border'}
    >
      <Spinner
        className={cnBase('absolute left-1/2 top-1/2 translate-x-1/2 -translate-y-1/2', !isPending ? 'opacity-0' : '')}
      />
      <ButtonText className={cnBase('text-foreground text-sm', isPending ? 'opacity-0' : '')}>
