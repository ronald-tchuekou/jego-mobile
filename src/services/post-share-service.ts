...
const PostShareService = {
  async getUserShare(userId: string, postId: string, token: string) {
    const { data, error } = await fetchHelper<{ data: PostShareModel }>(
      `/post-shares/${postId}/${userId}`,
      {
        method: "GET",
        headers: {
  ...
  async createOne(body: CreatePostShareDto, token: string) {
    ...
    if (error) throw new Error(error);
    if (!data?.data)
      throw new Error("Une erreur est survenue lors de la création du like.");
    return data.data;
  },
};
