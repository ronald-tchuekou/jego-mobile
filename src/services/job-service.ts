...
const JobService = {
  async count(companyId?: string) {
    const { data, error } = await fetchHelper<{ count: number }>(
      `/jobs/count?companyId=${companyId || ""}`,
      {
        headers: {
  ...
  async getAll(
    filter: FilterQuery & { status?: JobStatus; companyName?: string },
  ) {
  ...
  async getById(id: string) {
    const { data, error } = await fetchHelper<{ data: JobModel }>(
      `/jobs/${id}`,
    );
    if (error) throw new Error(error);
  ...
  async getExpired() {
    const { data, error } =
      await fetchHelper<PaginateResponse<JobModel>>(`/jobs/expired`);
    if (error) throw new Error(error);
    return data;
  },
  ...
  async getActive() {
    const { data, error } =
      await fetchHelper<PaginateResponse<JobModel>>(`/jobs/active`);
    if (error) throw new Error(error);
    return data;
  },
