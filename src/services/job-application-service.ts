...
const JobApplicationService = {
  async getAll(
    filter: FilterQuery & { status?: JobApplicationStatus },
    token: string,
  ) {
    ...
    const { data, error } = await fetchHelper<
      PaginateResponse<JobApplicationModel>
  ...
  async create(body: Partial<JobApplicationModel>, token: string) {
    const { data, error } = await fetchHelper<JobApplicationModel>(
      "/job-applications",
      {
        method: "POST",
  ...
  async getById(id: string, token: string) {
    const { data, error } = await fetchHelper<{ data: JobApplicationModel }>(
      `/job-applications/${id}`,
      {
        headers: {
          "Content-Type": "application/json",
  ...
  async getByJobId(jobId: string, filter: FilterQuery, token: string) {
    const query = objectToQueryString(filter);
    const { data, error } = await fetchHelper<PaginateResponse<JobModel>>(
      `/job-applications/job/${jobId}?${query}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  async getApplicationsCountPerDay(
    token: string,
