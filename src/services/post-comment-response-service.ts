...
const PostCommentResponseService = {
  async getPostCommentResponses(options: { postCommentId: string; token: string; page?: number; limit?: number }) {
  ...
  async createOne(body: CreatePostCommentResponseDto, token: string) {
    ...
    if (error) throw new Error(error)
    if (!data?.data) throw new Error('Une erreur est survenue lors de la création de la réponse au commentaire.')
    return data.data
  },

  async deleteOne(id: string, token: string) {
    const { data, error } = await fetchHelper<{ message: string }>(`/post-comment-responses/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
  ...
  async editOne(param: { id: string; comment: string }, token: string) {
    ...
    if (error) throw new Error(error)
    if (!data?.data) throw new Error('Une erreur est survenue lors de la modification de la réponse au commentaire.')
    return data.data
  },
}
