      ...
      <HeaderContainer>
        <HStack space='md'>
          ...
          <VStack className='flex-1'>
            ...
            <Text className='text-sm text-muted-foreground'>Informations complètes sur votre rendez-vous.</Text>
          </VStack>
        </HStack>
      </HeaderContainer>
      ...
      {isLoading ? (
        ...
        <ScrollView className='flex-1' contentContainerClassName='p-4'>
          ...
          <Card className='p-4 mt-4'>
            <Text className='text-base font-medium text-foreground mb-2'>Objet</Text>
            <Text className='text-sm text-foreground'>{appointment?.subject}</Text>
          </Card>

          {appointment?.content ? (
            <Card className='p-4 mt-4'>
              <Text className='text-base font-medium text-foreground mb-2'>Message</Text>
              <Text className='text-sm text-foreground'>{appointment?.content}</Text>
            </Card>
          ) : null}

          <Card className='p-4 mt-4'>
            <Text className='text-base font-medium text-foreground mb-2'>Entreprise</Text>
            <VStack space='sm'>
              {!!appointment?.company?.email && (
                <HStack className='items-center gap-2'>
                  <Icon as={IconMail} size='sm' className='text-muted-foreground' />
                  <Text className='text-sm text-muted-foreground'>{appointment?.company?.email}</Text>
                </HStack>
              )}
              {!!appointment?.company?.phone && (
                <HStack className='items-center gap-2'>
                  <Icon as={IconPhone} size='sm' className='text-muted-foreground' />
                  <Text className='text-sm text-muted-foreground'>{appointment?.company?.phone}</Text>
                </HStack>
              )}
            </VStack>
          </Card>
